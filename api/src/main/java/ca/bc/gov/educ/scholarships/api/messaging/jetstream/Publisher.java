...
public class Publisher {
  ...
  private void createOrUpdateInstituteEventStream(final Connection natsConnection) throws IOException, JetStreamApiException {
    val streamConfiguration = StreamConfiguration.builder().name(ApplicationProperties.STREAM_NAME).replicas(1).maxMessages(10000).addSubjects(SCHOLARSHIPS_EVENTS_TOPIC.toString()).build();
    try {
   ...
   */
  public void dispatchChoreographyEvent(final ScholarshipsEvent event) {
    if (event != null && event.getEventId() != null) {
      ...
      try {
        ...
        val pub = this.jetStream.publishAsync(SCHOLARSHIPS_EVENTS_TOPIC.toString(), JsonUtil.getJsonBytesFromObject(choreographedEvent));
